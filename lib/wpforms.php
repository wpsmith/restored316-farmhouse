...
/**
 * Creates a WPForms form if one added by a r316 theme does not exist.
 *
 * @since 2.10.0
 *
 * @return int|null ID of form if one exists or gets created. Null if form creation fails or WPForms is inactive.
 */
function r316_maybe_create_wpforms_form() { // phpcs:ignore -- r316 prefix for functions shared between themes.
	...
	$new_form_id = wpforms()->form->add(
		esc_html__( 'Simple Contact Form', 'farmhouse-theme' ),
		[],
		[
			'template' => 'contact',
			'builder'  => false,
 ...
 */
function r316_insert_contact_form( $content, $imported_posts ) { // phpcs:ignore -- r316 prefix for functions shared between themes.

	$form_id = r316_maybe_create_wpforms_form();
